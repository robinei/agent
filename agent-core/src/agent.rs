    ...
    match store.get_tree(tree_id) {
        Ok(Some(meta)) => {
            if let Some(repo_path) = &meta.repo_path {
                ...
                warn!(
                    "Repo path {:?} for tree {} does not exist, using agent dir",
                    repo_path, tree_id
                );
            }
        }
        Ok(None) => warn!("Tree {} not found, using agent dir", tree_id),
        Err(e) => warn!("Failed to get tree {}: {}, using agent dir", tree_id, e),
    }
